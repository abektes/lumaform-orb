...
`@lumaform/orb` is **published on npm**. Merging to `main` redeploys the studio (Railway) but never publishes. Pushing a `v*` tag does: `.github/workflows/publish.yml` publishes through npm trusted publishing (no token; npm trusts that exact file name, so don't rename it) and creates the GitHub release. That is irreversible; see [docs/RELEASING.md](docs/RELEASING.md). Never push a version tag unless asked. A change a runtime user would notice adds a line under `## [Unreleased]` in `packages/orb/CHANGELOG.md`. `main` is protected: direct pushes are refused, so work on a branch and open a pull request; CI's `test` check must pass before it can merge.
