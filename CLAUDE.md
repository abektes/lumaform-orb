...
`@lumaform/orb` is **published on npm**. Merging to `main` redeploys the studio (Railway) but never publishes. Pushing a `v*` tag stages a release: `.github/workflows/publish.yml` runs `npm stage publish` through npm trusted publishing (no token; npm trusts that exact file name, so don't rename it, and allows it only to stage) and creates the GitHub release. The version goes live only when the maintainer approves it on npmjs.com with 2FA, which is irreversible; see [docs/RELEASING.md](docs/RELEASING.md). Never push a version tag unless asked. A change a runtime user would notice adds a line under `## [Unreleased]` in `packages/orb/CHANGELOG.md`. `main` is protected: direct pushes are refused, so work on a branch and open a pull request; CI's `test` check must pass before it can merge.
